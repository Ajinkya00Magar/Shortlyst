...
const dbPath = path.join(dbDir, 'isro_hackathon.db');
...
const defaultProblemStatements = [
  {
    id: "PS-01",
...
// Helper to compute average metrics
const getAverageMetrics = (memberMetrics, teamMembers) => {
  const avg = {
    innovation: 0,
    difficulty: 0,
    feasibility: 0,
...
export function initDb() {
  db.exec(`
    CREATE TABLE IF NOT EXISTS problem_statements (
      ...
      ideas TEXT,
      challenges TEXT,
      metrics_innovation INTEGER,
      metrics_difficulty INTEGER,
      metrics_feasibility INTEGER,
      metrics_industryImpact INTEGER,
      metrics_learningOpportunity INTEGER,
      team_ratings TEXT,
      member_metrics TEXT
    )
  `);
  ...
  // Schema migration: Add member_metrics if table exists from legacy run
  try {
    db.exec("ALTER TABLE problem_statements ADD COLUMN member_metrics TEXT");
  } catch (e) {
    // Column already exists
  }

  // Seed default problem statements if empty
  const statementCountStmt = db.prepare("SELECT COUNT(*) as count FROM problem_statements");
  const { count: statementCount } = statementCountStmt.get();
  
  if (statementCount === 0) {
    const insertStmt = db.prepare(`
      INSERT INTO problem_statements (
        id, title, category, description, notes, pros, cons, ideas, challenges,
        metrics_innovation, metrics_difficulty, metrics_feasibility, metrics_industryImpact, metrics_learningOpportunity,
        team_ratings, member_metrics
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    defaultProblemStatements.forEach(ps => {
      // Initialize member metrics with default metrics for all 4 default members
      const initialMemberMetrics = {};
      defaultTeamMembers.forEach(member => {
        initialMemberMetrics[member] = {
          innovation: ps.metrics.innovation,
          difficulty: ps.metrics.difficulty,
          feasibility: ps.metrics.feasibility,
          industryImpact: ps.metrics.industryImpact,
          learningOpportunity: ps.metrics.learningOpportunity
        };
      });
      ...
      insertStmt.run(
        ...
        ps.description,
        ps.notes,
        JSON.stringify(ps.pros),
        JSON.stringify(ps.cons),
        ps.ideas,
        ps.challenges,
        ps.metrics.innovation,
        ps.metrics.difficulty,
        ps.metrics.feasibility,
        ps.metrics.industryImpact,
        ps.metrics.learningOpportunity,
        JSON.stringify(ps.teamRatings),
        JSON.stringify(initialMemberMetrics)
      );
    });
  }

  // Seed default team members if empty
  const membersCountStmt = db.prepare("SELECT COUNT(*) as count FROM team_members");
  const { count: membersCount } = membersCountStmt.get();

  if (membersCount === 0) {
    const insertMemberStmt = db.prepare("INSERT INTO team_members (name, display_order) VALUES (?, ?)");
    defaultTeamMembers.forEach((name, i) => {
      insertMemberStmt.run(name, i);
    });
  } else {
    // If the jury members are currently in DB from a legacy run, let's make sure we update them to the 4 specified members
    const getMembersStmt = db.prepare("SELECT name FROM team_members ORDER BY display_order ASC");
    const existing = getMembersStmt.all().map(r => r.name);
    
    // Check if the current list in DB matches default list. If not, reset it so it uses the correct 4 jury members.
    const isMatched = existing.length === defaultTeamMembers.length && existing.every((v, i) => v === defaultTeamMembers[i]);
    if (!isMatched) {
      updateTeamMembers(defaultTeamMembers);
    }
  }
}

// Convert SQLite row back to problem statement format expected by React
const mapRowToProblemStatement = (row) => {
  let memberMetrics = {};
  try {
    memberMetrics = JSON.parse(row.member_metrics || "{}");
  } catch (e) {
    memberMetrics = {};
  }
  ...
  // Backfill if empty
  const legacyMetrics = {
    innovation: row.metrics_innovation || 3,
    difficulty: row.metrics_difficulty || 3,
    feasibility: row.metrics_feasibility || 3,
    industryImpact: row.metrics_industryImpact || 3,
    learningOpportunity: row.metrics_learningOpportunity || 3
  };

  defaultTeamMembers.forEach(m => {
    if (!memberMetrics[m]) {
      memberMetrics[m] = { ...legacyMetrics };
    }
  });

  const avgMetrics = getAverageMetrics(memberMetrics, defaultTeamMembers);
  ...
  return {
    ...
    teamRatings: JSON.parse(row.team_ratings || "[]"),
    memberMetrics,
    metrics: avgMetrics
  };
};
...
export function getProblemStatements() {
  const stmt = db.prepare("SELECT * FROM problem_statements ORDER BY id");
  const rows = stmt.all();
  return rows.map(mapRowToProblemStatement);
}

export function updateProblemStatement(id, fields) {
  const sets = [];
  const params = [];
  ...
  if ('pros' in fields) {
    sets.push('pros = ?');
    params.push(JSON.stringify(fields.pros));
  }
  if ('cons' in fields) {
    sets.push('cons = ?');
    params.push(JSON.stringify(fields.cons));
  }
  if ('ideas' in fields) {
    sets.push('ideas = ?');
    params.push(fields.ideas);
  }
  if ('challenges' in fields) {
    sets.push('challenges = ?');
    params.push(fields.challenges);
  }
  if ('teamRatings' in fields) {
    sets.push('team_ratings = ?');
    params.push(JSON.stringify(fields.teamRatings));
  }
  if ('memberMetrics' in fields) {
    sets.push('member_metrics = ?');
    params.push(JSON.stringify(fields.memberMetrics));

    // Calculate dynamic averages and sync to legacy metrics columns
    const avg = getAverageMetrics(fields.memberMetrics, defaultTeamMembers);
    sets.push('metrics_innovation = ?');
    params.push(avg.innovation);
    sets.push('metrics_difficulty = ?');
    params.push(avg.difficulty);
    sets.push('metrics_feasibility = ?');
    params.push(avg.feasibility);
    sets.push('metrics_industryImpact = ?');
    params.push(avg.industryImpact);
    sets.push('metrics_learningOpportunity = ?');
    params.push(avg.learningOpportunity);
  }

  if (sets.length === 0) return;

  params.push(id);
  const stmt = db.prepare(`UPDATE problem_statements SET ${sets.join(', ')} WHERE id = ?`);
  stmt.run(...params);
}

export function getTeamMembers() {
  const stmt = db.prepare("SELECT name FROM team_members ORDER BY display_order ASC");
  const rows = stmt.all();
  return rows.map(r => r.name);
}

export function updateTeamMembers(membersList) {
  // Clear members
  db.exec("DELETE FROM team_members");
  
  // Re-insert members
  const insertStmt = db.prepare("INSERT INTO team_members (name, display_order) VALUES (?, ?)");
  membersList.forEach((name, i) => {
    insertStmt.run(name, i);
  });
}

export function resetDatabase() {
  // Drop tables
  db.exec("DROP TABLE IF EXISTS problem_statements");
